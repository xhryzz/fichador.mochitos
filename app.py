...
def _job_notify_open_record(now=None, force=False):
    ...
    for u in users:
        ...
        # Si la condición se cumple (o si se forza el envío) y no se ha enviado ya hoy
...
def admin_user_dashboard(user_id):
    ...
    # Horarios
    schedules = Schedule.query.filter_by(user_id=user_id).order_by(Schedule.day_of_week).all()
    schedules_by_day = {s.day_of_week: s for s in schedules}
    days_of_week = [(0, 'Lunes'), (1, 'Martes'), (2, 'Miércoles'), (3, 'Jueves'), (4, 'Viernes'), (5, 'Sábado'), (6, 'Domingo')]
    
    # Últimos fichajes (para la tabla)
    recent_records = (TimeRecord.query
                      .filter_by(user_id=user_id)
                      .order_by(TimeRecord.date.desc(), TimeRecord.entry_time.desc())
                      .limit(10)
                      .all())
    ...
    # Calcular horas requeridas semanales
    weekly_required_hours = 0
    for i in range(7):
        day = week_start + timedelta(days=i)
        day_of_week = day.weekday()
        day_schedule = next((s for s in schedules if s.day_of_week == day_of_week and s.is_active), None)
        if day_schedule:
            weekly_required_hours += day_schedule.hours_required
    
    return render_template('admin_user_dashboard.html',
                           user=user,
                           active_record=active_record,
                           today_records=today_records,
                           ...
                           weekly_required_hours=weekly_required_hours,
                           total_hours_worked=total_hours_worked,
                           total_hours_required=user.total_hours_required,
...
@app.route('/admin/user/<int:user_id>/clock_out', methods=['POST'])
@login_required
def admin_clock_out_user(user_id):
    if not current_user.is_admin:
        flash('No tienes permisos de administrador', 'error')
        return redirect(url_for('dashboard'))
    ...
    now_loc = now_local()
    today = now_loc.date()
    active_record = TimeRecord.query.filter_by(user_id=user_id, date=today, exit_time=None).first()

    if not active_record:
        flash('No hay una sesión activa para cerrar', 'error')
        return redirect(url_for('admin_user_dashboard', user_id=user_id))

    active_record.exit_time = now_loc.astimezone(timezone.utc)
    active_record.is_active = False
    db.session.commit()
    
    flash(f'Sesión cerrada correctamente para {user.name}', 'success')
    return redirect(url_for('admin_user_dashboard', user_id=user_id))

@app.route('/admin/user/<int:user_id>/schedule/add', methods=['POST'])
@login_required
def admin_add_schedule_for_user(user_id):
    if not current_user.is_admin:
        flash('No tienes permisos de administrador', 'error')
        return redirect(url_for('dashboard'))
    ...
    existing = Schedule.query.filter_by(user_id=user_id, day_of_week=day_of_week).first()
    if existing:
        existing.start_time = start_time
        existing.end_time = end_time
        existing.start_time_2 = start_time_2
...
@app.route('/admin/user/<int:user_id>/schedule/toggle/<int:id>')
@login_required
def admin_schedule_toggle(user_id, id):
    if not current_user.is_admin:
        flash('No tienes permisos de administrador', 'error')
        return redirect(url_for('dashboard'))
    ...
    if schedule.user_id != user_id:
        flash('Horario no pertenece al usuario', 'error')
        return redirect(url_for('admin_user_dashboard', user_id=user_id))
...
@app.route('/admin/user/<int:user_id>/schedule/delete/<int:id>')
@login_required
def admin_schedule_delete(user_id, id):
    if not current_user.is_admin:
        flash('No tienes permisos de administrador', 'error')
        return redirect(url_for('dashboard'))
...
@app.route('/admin/user/<int:user_id>/record/add', methods=['POST'])
@login_required
def admin_add_record_for_user(user_id):
    if not current_user.is_admin:
        flash('No tienes permisos de administrador', 'error')
        return redirect(url_for('dashboard'))
    ...
    try:
        ...
        db.session.add(record)
        db.session.commit()
        flash('Fichaje añadido correctamente', 'success')
    except Exception as e:
        db.session.rollback()
        flash('Error al añadir el fichaje', 'error')
